...
export class recibeFirmaScreen extends connect(store, MEDIA_CHANGE, SCREEN)(LitElement) {
	constructor() {
		super();
		this.hidden = true;
		this.area = "body";
		...
		return html`
			...
			<div id="datos" class="grid row">
				...
				<div class="grid column" style="padding:0;margin:0;">
					...
					<button id="btnseguir" flat @click="${this.seguir}">FINALIZAR</button>
				</div>
			</div>
		`;
	}
	stateChanged(state, name) {
		if (name == SCREEN || name == MEDIA_CHANGE) {
			...
			const SeMuestraEnUnasDeEstasPantallas = "-recibeFirma-".indexOf("-" + state.screen.name + "-") != -1;
...
window.customElements.define("recibe-firma-screen", recibeFirmaScreen);
